...
# However, it seems to be scaled differently potentially???
freq <- dataset_variables$Frequency[dataset_variables$Name == "Mauny_Goat"]
stride_window <- dataset_variables$StrideWindow[dataset_variables$Name == "Mauny_Goat"]
list_locomotion_labels <- c("walking")

# Data --------------------------------------------------------------------
dat <- fread(file.path(base_path, "Data/Accelerometer/Mauny_Goat/Mauny_Goat_formatted.csv"))
...
# add mass -------------------------------------------------------
animal_mass <- fread(file.path(base_path, "Data/Accelerometer/Mauny_Goat", "Mass_of_Individuals.csv"))
...
fwrite(summ_stats, file.path(base_path, "Output/Mauny_GoatAccelerometer_summary_stats.csv"))
...
# save
img_output <- file.path(base_path, "Output", "Mauny_GoatAccelerometer.png")
...
# static <- static %>% dplyr::filter(ID == "21055")
# ggplot(static, aes(x = seq(1:nrow(static)))) + 
#   geom_path(aes(y = X, colour = "X")) + 
#   geom_path(aes(y = Y, colour = "Y")) + 
#   geom_path(aes(y = Z, colour = "Z")) + 
#   facet_grid(~ID, scales = "free")
