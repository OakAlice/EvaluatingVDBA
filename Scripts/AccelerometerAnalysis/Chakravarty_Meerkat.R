# Chakravarty_Meerkat analysis --------------------------------------------

freq <- dataset_variables$Frequency[dataset_variables$Name == "Chakravarty_Meerkat"]
stride_window <- dataset_variables$StrideWindow[dataset_variables$Name == "Chakravarty_Meerkat"]
list_locomotion_labels <- c("Running")


# Set up ------------------------------------------------------------------
data <- fread(file.path(base_path, "Data/Accelerometer/Chakravarty_Meerkat/labelledTriaxialAccData.mat")) %>%
  rename(X = acc_x, Y = acc_y, Z = acc_z, Activity = behavior)
ID <- fread(file.path(base_path, "Data/Accelerometer/Chakravarty_Meerkat/raw/matched_acceleration_metadata_out.csv"))
...
# Add animal mass ---------------------------------------------------------
animal_mass <- dataset_variables$LogMass[dataset_variables$Name == "Chakravarty_Meerkat"]
...
fwrite(summ_stats, file.path(base_path, "Output/Chakravarty_MeerkatAccelerometer_summary_stats.csv"))
