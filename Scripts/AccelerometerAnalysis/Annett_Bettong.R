# Analysis of Jasmin Annett's Bettong data --------------------------------
freq <- dataset_variables$Frequency[dataset_variables$Name == "Annett_Bettong"]
stride_window <- dataset_variables$StrideWindow[dataset_variables$Name == "Annett_Bettong"]
list_locomotion_labels <- c("Locomotion")

# Prepare data ------------------------------------------------------------
files <- list.files(file.path(base_path, "Data/Accelerometer/Annett_Bettong/raw"), full.names = TRUE)
...
# Add animal mass ---------------------------------------------------------
animal_mass <- fread(file.path(base_path, "Data/Accelerometer/Annett_Bettong", "Mass_of_Individuals.csv")) %>%
  pull(LogMass)
...
fwrite(summ_stats, file.path(base_path, "Output/Annett_BettongAccelerometer_summary_stats.csv"))
