...
# order the data based on animal size
species <- dataset_variables %>% 
  dplyr::filter(Type == "Mam", Zone == "Land") %>%
  arrange(LogMass) %>%
  pull(Name)
all_data$Species <- factor(all_data$Species, levels = species)
...
ggplot(selected_data, aes(x = seconds_VDBA, fill = Species)) + 
