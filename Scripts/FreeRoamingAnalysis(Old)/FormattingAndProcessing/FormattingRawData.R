...
if (species == "Sparkes_Koala"){
  ...
  data <- fread(file.path(base_path, "/Data/AccelerometerData/Sparkes_Koala/raw/labelled_data.csv"))
  data <- data[, c(1:4, 8)]
...
fwrite(data, file.path(base_path, "Data", "AccelerometerData", species, paste0(species, "_reformatted.csv")))
...
# The other datasets ------------------------------------------------------
# I am no longer using these datasets, but have retained legacy code here
...
#   rm(dfs)
#   
# } else if (species %in% c("Smit_Cat", "Studd_Squirrel", "HarveyCaroll_Pangolin", "Vehkaoja_Dog")){
#   data <- fread(file.path(base_path, "AccelerometerData", species, "raw", 
